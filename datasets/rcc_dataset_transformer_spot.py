...
class RCCDataset(Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        for i, img in enumerate(self.d_feats):
            pre_insert, _ = img.split('.n')
            idx = int(os.path.splitext(pre_insert)[0].split('_')[-1])
            if i != idx:
                self.d_feats.insert(i, 'none')
        self.s_feats = sorted(os.listdir(self.s_feat_dir))
        for i, img in enumerate(self.s_feats):
            pre_insert, _ = img.split('.n')
            idx = int(os.path.splitext(pre_insert)[0].split('_')[-1])

            if i != idx:
                self.s_feats.insert(i, 'none')
        ...
        for i, img in enumerate(self.d_imgs):
            idx = int(os.path.splitext(img)[0].split('_')[-1])
            if i != idx:
                self.d_imgs.insert(i, 'none')
        self.s_imgs = sorted(os.listdir(self.s_img_dir))
        for i, img in enumerate(self.s_imgs):
            idx = int(os.path.splitext(img)[0].split('_')[-1])
            if i != idx:
                self.s_imgs.insert(i, 'none')
    ...
    def __getitem__(self, index):
        ...
        if seq.size == self.max_seq_length:
            ...
            # pos_with_ignore = np.zeros_like(pos)
...
def rcc_collate(batch):
    ...
    # pos_batch = default_collate(transposed[4])
    # pos_with_ignore_batch = default_collate(transposed[5])
