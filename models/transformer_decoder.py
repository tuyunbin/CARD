...
class DynamicCore(nn.Module):
    ...
    def forward(self, seq, dec_mask, encoder_output,
        ...
        enc_outputs = encoder_output.transpose(0, 1)
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(dec_hidden_states)
            all_att_vis.append(attention_vis)
        return all_encoder_layers[-1].transpose(0, 1), all_att_vis[-1]
...
class DynamicSpeaker(nn.Module):
    ...
    def sample(
            self,
            encoder_output, start_idx=2, unk_idx=1, sample_max=0):
        ...
        text_masks = encoder_output.new_zeros(bsz, max_cap_len).float()  # zeros
